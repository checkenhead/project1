...
import { VALIDATE_RESULT } from '@/util/constance/common'
import { objUtil } from '@/util/function/common'

// export type CustomStateType<T extends Record<keyof T, unknown>> = { [P in keyof T]: T[P] }
export type CustomStateType<T> = { [P in keyof T]: T[P] }
export type ValidationType<T> = Record<keyof T, { isValid?: boolean; msg?: string }>
...
const useCustomState = <T,>(initState = {} as CustomStateType<T>) => {
  ...
  const initResult: ValidationType<T> = useMemo(
    () => Object.assign({}, ...keys.map((key) => ({ [key]: undefined }))),
    [keys]
  )
    ...
    () =>
      objUtil.map(state, (key, value) => ({
        [key]: (validator: ValidatorType<T, typeof value>) => {
          ...
          setResult((prev) => {
            if (validateResult === VALIDATE_RESULT.UNCHECK) return { ...prev, [key]: undefined }
            return isValid ? { ...prev, [key]: { isValid } } : { ...prev, [key]: { isValid, msg: result } }
          })
  ...
  const checkAllState: CheckAllStateType<T> = useCallback(
    (validator) => {
      ...
      const newResult = validateResults.map((result, index) => {
        if (result === VALIDATE_RESULT.UNCHECK) return { [keys[index]]: undefined }

        const isValid = result === VALIDATE_RESULT.OK
        return isValid ? { [keys[index]]: { isValid } } : { [keys[index]]: { isValid, msg: result } }
      })
    ...
    () =>
      objUtil.map(state, (key) => ({
        [key]: (result: ValidateReturnType) => {
          setResult((prev) => {
            if (result === VALIDATE_RESULT.UNCHECK) return { ...prev, [key]: undefined }
  ...
  const resetResult: ResetResultType<T> = useCallback((...keys) => {
    if (keys.length === 0) {
      ...
      setResult((prev) => ({ ...prev, ...Object.assign({}, ...keys.map((key) => ({ [key]: undefined }))) }))
    }
  }, [])
